...
router.delete('/logo', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Get current profile
    const result = await client.query('SELECT id, logo_url FROM business_profile ORDER BY id DESC LIMIT 1');
    ...
    if (logoUrl) {
      ...
      await client.query('UPDATE business_profile SET logo_url = NULL WHERE id = $1', [
        result.rows[0].id,
      ]);
    }
