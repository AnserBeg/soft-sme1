  ...
  try {
    ...
    for (const salesOrder of salesOrders) {
      try {
        ...
        // Get time entries for this sales order (only need total hours)
        ...
        console.log(`  📅 Time entries: ${totalHours} hours`);
        ...
        if (totalHours > 0) {
          ...
          const labourRate = labourRateResult.rows.length > 0 ? 
            parseFloat(labourRateResult.rows[0].value) : 60; // Default to $60 if not set
          
          // Calculate correct amounts using GLOBAL rates, not time entry averages
          const labourAmount = totalHours * labourRate;
          ...
          `, [labourAmount, labourRate, totalHours, salesOrder.sales_order_id]);
