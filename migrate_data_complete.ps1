# Complete Data Migration Script for PostgreSQL
# This script handles ALL schema issues and ensures complete data migration

$env:PGPASSWORD = "123"

Write-Host "Starting complete data migration..." -ForegroundColor Green
...
try {
    ...
    # Export inventory (add missing columns)
    psql -h localhost -p 5432 -U postgres -d soft_sme_db -c "`\copy (SELECT part_number, part_description, unit, last_unit_cost, quantity_on_hand, created_at, updated_at, reorder_point, part_type, 'General' as category, NULL as part_id FROM inventory) TO 'C:\temp\inventory.csv' WITH CSV HEADER"
    ...
    # Export sales orders (remove extra column)
