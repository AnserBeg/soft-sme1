# Final Data Migration Script for PostgreSQL
# This script handles all remaining schema issues

$env:PGPASSWORD = "123"

Write-Host "Starting final data migration..." -ForegroundColor Green
...
try {
    ...
    psql -h localhost -p 5432 -U postgres -d soft_sme_db -c "`\copy (SELECT id, username, email, password_hash, company_id, access_role, NULLIF(created_at, '1') as created_at, NULLIF(updated_at, '1') as updated_at, role, force_password_change, preferred_device_id, last_login_at, last_login_ip FROM users) TO 'C:\temp\users.csv' WITH CSV HEADER"
    ...
    # Export inventory (add default category)
    psql -h localhost -p 5432 -U postgres -d soft_sme_db -c "`\copy (SELECT part_number, part_description, unit, last_unit_cost, quantity_on_hand, created_at, updated_at, reorder_point, part_type, 'General' as category FROM inventory) TO 'C:\temp\inventory.csv' WITH CSV HEADER"
    ...
    # Export purchase orders (add default qbo_export_date)
    ...
    # Export sales orders (add default qbo_export_date)
    psql -h localhost -p 5432 -U postgres -d soft_sme_db -c "`\copy (SELECT sales_order_id, sales_order_number, customer_id, sales_date, product_name, product_description, subtotal, total_gst_amount, total_amount, status, estimated_cost, default_hourly_rate, created_at, updated_at, exported_to_qbo, qbo_exported_date, qbo_export_status, sequence_number, customer_po_number, vin_number, quote_id, terms, qbo_invoice_id, NULL as qbo_export_date FROM salesorderhistory) TO 'C:\temp\salesorderhistory.csv' WITH CSV HEADER"
