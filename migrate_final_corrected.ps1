...
# This script matches the staging database column order exactly

$env:PGPASSWORD = "123"

Write-Host "Starting final corrected migration..." -ForegroundColor Green
...
try {
    # Export sales orders with EXACT column order matching staging database
    psql -h localhost -p 5432 -U postgres -d soft_sme_db -c "`\copy (SELECT sales_order_id, sales_order_number, customer_id, sales_date, product_name, product_description, subtotal, total_gst_amount, total_amount, status, estimated_cost, default_hourly_rate, created_at, updated_at, customer_po_number, vin_number, exported_to_qbo, qbo_invoice_id, NULL as qbo_export_date, qbo_export_status, quote_id, sequence_number, terms FROM salesorderhistory) TO 'C:\temp\salesorderhistory_corrected.csv' WITH CSV HEADER"
    Write-Host "✓ Sales orders exported successfully (corrected column order)" -ForegroundColor Green
...
# Import sales orders (with corrected column order)
try {
    psql -h localhost -p 5432 -U postgres -d soft_sme_db_staging -c "`\copy salesorderhistory FROM 'C:\temp\salesorderhistory_corrected.csv' WITH CSV HEADER"
...
Write-Host "Note: qbo_export_date column was set to NULL due to timestamp issues." -ForegroundColor Yellow
