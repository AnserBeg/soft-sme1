# Sales Orders Migration Fix
# This script specifically handles sales orders with timestamp issues

$env:PGPASSWORD = "123"

Write-Host "Starting sales orders migration fix..." -ForegroundColor Green
...
try {
    # Export sales orders with better timestamp handling
    psql -h localhost -p 5432 -U postgres -d soft_sme_db -c "`\copy (SELECT sales_order_id, sales_order_number, customer_id, sales_date, product_name, product_description, subtotal, total_gst_amount, total_amount, status, estimated_cost, default_hourly_rate, created_at, updated_at, exported_to_qbo, CASE WHEN qbo_exported_date IS NULL OR qbo_exported_date = '' OR qbo_exported_date = 'NULL' THEN NULL ELSE qbo_exported_date END as qbo_exported_date, qbo_export_status, sequence_number, customer_po_number, vin_number, quote_id, terms, qbo_invoice_id FROM salesorderhistory) TO 'C:\temp\salesorderhistory_fixed.csv' WITH CSV HEADER"
    Write-Host "✓ Sales orders exported with fixed timestamps" -ForegroundColor Green
...
try {
    psql -h localhost -p 5432 -U postgres -d soft_sme_db_staging -c "`\copy salesorderhistory FROM 'C:\temp\salesorderhistory_fixed.csv' WITH CSV HEADER"
...
Write-Host "`nSales orders migration fix completed!" -ForegroundColor Green
