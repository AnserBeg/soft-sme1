...
const SupplyPage: React.FC = () => {
  ...
  // Handle cell edit commit (for reorder_point and last_unit_cost)
  const processRowUpdate = async (newRow: any, oldRow: any) => {
    ...
    try {
      // This PUT request now sends either reorder_point, last_unit_cost, or both
      const response = await api.put(`/api/inventory/${encodeURIComponent(partNumber)}`, updatedFields); // Use api instead of axios

      console.log(`Supply inventory updated for part ${partNumber}:`, response.data);
      ...
      console.error(`Error updating supply inventory for part ${partNumber}:`, error);
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      <Dialog open={editDialogOpen} onClose={handleCloseEditDialog} maxWidth="md" fullWidth>
        <DialogTitle>Edit Supply Part</DialogTitle>
        <DialogContent>
          {editPart && (
