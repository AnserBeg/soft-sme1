...
import { getServiceInventory, previewCleanupEnforce, applyCleanupEnforce } from '../services/inventoryService';
...
const ServicePage: React.FC = () => {
  ...
  // Fetch inventory data from backend (only service items)
  const fetchInventory = async () => {
    setLoading(true);
    try {
      const response = await getServiceInventory();
  ...
  useEffect(() => {
    fetchInventory();
    const handleServiceUpdated = () => fetchInventory();
    window.addEventListener('service-updated', handleServiceUpdated);
    return () => window.removeEventListener('service-updated', handleServiceUpdated);
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'part_type', headerName: 'Type', flex: 0.5, headerAlign: 'left' },
    {
      ...
      type: 'number',
      valueGetter: (params) => {
        // For service items, quantity_on_hand is "NA", so value is 0
  ...
  const processRowUpdate = async (newRow: any, oldRow: any) => {
    ...
    try {
      ...
      console.log(`Service inventory updated for part ${apiPartNumber}:`, response.data);
      ...
      console.error(`Error updating service inventory for part ${apiPartNumber}:`, error);
  ...
  const handleExportCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', 'service_list.csv');
  ...
  const handlePrintPDF = () => {
    ...
    window.open('/api/inventory/export/pdf?partType=service', '_blank');
  };
  ...
  // New: Enforce rules and show potential duplicates, allow merging (service)
  const handleCleanup = async () => {
    setCleaning(true);
    try {
      const preview = await previewCleanupEnforce('service');
      setCleanupPreview(preview?.preview || preview);
      setShowCleanupPreview(true);
    } catch (error) {
      console.error('Error during cleanup preview:', error);
      alert('Failed to analyze service for cleanup.');
    } finally {
      setCleaning(false);
    }
  };
  ...
  const applyCleanup = async () => {
    ...
    setCleaning(true);
    try {
      const applied = await applyCleanupEnforce('service', merges);
  ...
  const handleRowClick = (params: any) => {
    setEditingPart({
      ...
      part_type: params.row.part_type || 'service',
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      <UnifiedPartDialog
        ...
        title={editingPart ? 'Edit Service Part' : 'Add New Service Part'}
      />
